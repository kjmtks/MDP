...
interface ConnectDialogProps {
  ...
  channelId: string;
  token?: string;
  ipCandidates?: { name: string; address: string }[];
  port?: number | null;
}

export const ConnectDialog: React.FC<ConnectDialogProps> = ({ open, onClose, channelId, token, ipCandidates, port: embeddedPort }) => {
  const [copied, setCopied] = useState(false);
  const [qrCodeUrl, setQrCodeUrl] = useState('');

  const isEmbedded = !!(ipCandidates && ipCandidates.length && embeddedPort);

  // In Electron the app runs from file://, so default to the embedded server's LAN IP + port.
  const [hostOverride, setHostOverride] = useState<string | null>(null);
  const [portOverride, setPortOverride] = useState<string | null>(null);
  const hostname = hostOverride ?? (isEmbedded ? ipCandidates![0].address : window.location.hostname);
  const port = portOverride ?? (isEmbedded ? String(embeddedPort) : window.location.port);
  const setHostname = setHostOverride;
  const setPort = setPortOverride;
        ...
        <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 4, py: 2 }}>
          <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            ...
            <Box sx={{ mt: 2, width: '100%' }}>
              ...
              {isEmbedded && ipCandidates!.length > 0 && (
                <TextField
                  select
                  label="Network (IP)"
                  size="small"
                ...
                >
                  {ipCandidates!.map((ip) => (
                    <MenuItem key={ip.address} value={ip.address}>{ip.address} — {ip.name}</MenuItem>
                  ))}
                </TextField>
              )}
