...
import './DrawingPalette.css';

interface DrawingPaletteProps {
  toolType: 'pen' | 'eraser' | 'select';
  setToolType: (type: 'pen' | 'eraser' | 'select') => void;
  color: string;
...
export const DrawingPalette: React.FC<DrawingPaletteProps> = ({
  ...
  container, style, className, stylusOnly, setStylusOnly
}) => {
  const [anchorEl, setAnchorEl] = React.useState<HTMLButtonElement | null>(null);

  const handleColorClick = (event: React.MouseEvent<HTMLButtonElement>) => { setAnchorEl(event.currentTarget); };
  ...
  return (
    <div
      ...
      style={{
        ...
        backdropFilter: 'blur(5px)', border: '1px solid #444',
        pointerEvents: 'auto', ...style
      }}
    >
      <Tooltip title="Select & Move">
        <IconButton
          color={toolType === 'select' ? "primary" : "default"}
          onClick={() => setToolType('select')}
          style={{ backgroundColor: toolType === 'select' ? 'rgba(25, 118, 210, 0.2)' : 'transparent' }}
        >
          <NearMeIcon />
        </IconButton>
      </Tooltip>
      ...
      <div className="width-selector" style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
        {WIDTHS.map(w => (
          <button
            key={w} onClick={() => setLineWidth(w)} type="button" className="palette-control"
            style={{
              ...
              padding: 0, outline: 'none', appearance: 'none', WebkitAppearance: 'none',
            }}
            title={`Width: ${w}px`}
          />
        ))}
      ...
      {setStylusOnly && (
        <>
          <Tooltip title={stylusOnly ? "Stylus Only (Touch disabled)" : "Touch Drawing Enabled"}>
            <IconButton onClick={() => setStylusOnly(!stylusOnly)} style={{ color: stylusOnly ? '#ff6600' : '#888' }}>
      ...
      <Tooltip title="Undo (Ctrl+Z)"><span><IconButton disabled={!canUndo} onClick={onUndo} style={{ color: '#888' }} ><UndoIcon /></IconButton></span></Tooltip>
      <Tooltip title="Redo (Ctrl+Y)"><span><IconButton disabled={!canRedo} onClick={onRedo} style={{ color: '#888' }}><RedoIcon /></IconButton></span></Tooltip>
      <Tooltip title="Clear All"><IconButton color="error" onClick={onClear}><DeleteIcon /></IconButton></Tooltip>

      <Popover
        open={Boolean(anchorEl)} anchorEl={anchorEl} onClose={handleColorClose}
        anchorOrigin={{ vertical: 'top', horizontal: 'center' }} transformOrigin={{ vertical: 'bottom', horizontal: 'center' }}
        container={targetContainer} sx={{ zIndex: 11000 }}
      >
        <div className="palette-popover-content" style={{ padding: 10, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, backgroundColor: '#333' }}>
          {COLORS.map(c => (
            <button
              key={c} onClick={() => selectColor(c)} type="button" className="palette-control"
              style={{ width: 32, height: 32, backgroundColor: c, borderRadius: '50%', cursor: 'pointer', border: '1px solid #666', padding: 0, outline: 'none', appearance: 'none', WebkitAppearance: 'none' }}
            />
          ))}
          <input type="color" value={color} onChange={(e) => { setColor(e.target.value); setToolType('pen'); }} style={{ width: 32, height: 32, padding: 0, border: 'none', background: 'none', cursor: 'pointer' }} />
        </div>
      </Popover>
    </div>
  );
